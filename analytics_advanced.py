...
def calculate_edge_validation(results: List, min_bets: int = 10) -> Dict[str, Any]:
    ...
    if valid_buckets:
        ...
        overall_accuracy = sum(
            b['accuracy'] * b['num_bets'] for b in valid_buckets
        ) / total_bets
    else:
...
def calculate_model_trends(results: List, window_days: int = 30, num_periods: int = 8) -> Dict[str, Any]:
    """
    ...
    for i in range(num_periods):
        ...
        avg_confidence = sum(
            r['confidence_score'] if 'confidence_score' in r.keys() and r['confidence_score'] is not None else 0
...
def calculate_drawdown_metrics(capital_history: List[Tuple[datetime, float]]) -> Dict[str, Any]:
    """
    ...
    return {
        ...
        'max_dd_recovery_days': max_dd_period['recovery_days'] if max_dd_period else 0,
        'status': status,
