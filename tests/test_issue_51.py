...
class TestIssue51(unittest.TestCase):
    ...
    def test_timestamp_converter_in_new_thread(self):
        ...
        def run_test(result_container):
            try:
                with get_db_connection() as session:
                    ...
                    session.execute(text("CREATE TEMPORARY TABLE IF NOT EXISTS test_issue_51 (ts TIMESTAMP)"))
                    ...
                    session.execute(text("INSERT INTO test_issue_51 (ts) VALUES (:ts)"), {"ts": "2023-10-27"})
                    session.commit()

                    # Read back
                    result = session.execute(text("SELECT ts FROM test_issue_51")).fetchone()
                    if result:
                        val = result[0]
        ...
        self.assertIsInstance(val, datetime.datetime, f"Expected datetime object, got {type(val)}: {val}")
